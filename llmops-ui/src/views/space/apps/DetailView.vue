...
const send = async () => {
  ...
  if (isLoading.value) {
    Message.warning('请稍后，模型正在处理上一条消息')
    return
  }
      ...
      <div class="flex flex-col w-1/3 bg-white h-full">
        ...
        <div class="h-full min-h-0 px-6 py-7 overflow-x-hidden overflow-y-scroll scrollbar-w-none">
          ...
          <div class="flex flex-row gap-2 mb-6" v-for="message in messages" :key="message.content">
            ...
            <div class="flex flex-col gap-2">
              <div class="font-semibold text-gray-700">
                {{ message.role === 'human' ? '陈小课' : 'DeepSeek聊天助手' }}
              </div>
              <div
                v-if="message.role === 'human'"
          ...
          >
            ...
            <div class="text-2xl font-bold text-gray-900 mt-2">DeepSeek聊天机器人</div>
          </div>
          <!-- AI加载状态 -->
          <div v-if="isLoading" class="flex flex-row gap-2 mb-6">
            ...
            <!-- 实际消息 -->
            <div class="flex flex-col gap-2">
              <div class="font-semibold text-gray-700">DeepSeek聊天机器人</div>
              <div
                class="max-w-max bg-gray-100 text-gray-900 border border-gray-200 px-4 py-3 rounded-2xl leading-5"
              >
                <icon-loading />
        ...
        <div class="w-full flex-shrink-0 flex flex-col">
          ...
          <div class="px-6 flex items-center gap-4">
            <!-- 清除按钮 -->
            <a-button class="flex-shrink-0" type="text" shape="circle" @click="clearHistory">
              <template #icon>
                <icon-empty size="16" :style="{ color: '#374251' }" />
              </template>
            </a-button>
            <!-- 输入框 -->
            <div
            ...
            >
              ...
              <a-button type="text" shape="circle">
                <template #icon>
                  <icon-plus-circle size="16" :style="{ color: '#374251' }"></icon-plus-circle>
                </template>
              </a-button>
